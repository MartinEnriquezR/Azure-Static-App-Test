...
export default {
  ...
  methods: {
    ...
    procesar() {
      let fechaDenuncia;
      if (this.usuaria.fecha == "") {
        fechaDenuncia = null;
      } else {
        fechaDenuncia = this.usuaria.fecha;
      }
      let json = {
      ...
      axios({
        url: "https://backend-upiita.herokuapp.com/cuestionario/",
        method: "post",
        data: json,
        headers: {
          Authorization: token,
        ...
        .then((data) => {
          ...
          Swal.fire({
            icon: "success",
            title: "Cuestionario registrado con éxito",
        ...
        .catch((error) => {
          ...
          Swal.fire({
            icon: "error",
            title: "Aún no puedes contestar el Cuestionario",
            text:
              "Debes esperar al menos 10 minutos después de la activación de la alerta",
            confirmButtonColor: "#a5b232",
          });
